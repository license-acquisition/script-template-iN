...
day = date.today().day
'''
#f = codecs.open('eng_c_OHpen_%s%s%s_000.csv' %(str(year), str(month).zfill(2), str(day).zfill(2)), 'a', 'utf-8')
keywords = [''.join(i) for i in product(ascii_uppercase, repeat =2)]
l=codecs.open("OH_eng_Links.csv","w","utf-8")
browser  = webdriver.Chrome()

for x in keywords:
    try:
        browser.get("https://license.ohio.gov/Lookup/")
        browser.find_element_by_css_selector("#iform > table > tbody > tr > td > table:nth-child(5) > tbody > tr:nth-child(1) > td:nth-child(2) > select > option:nth-child(11)").click()
        search = browser.find_element_by_css_selector("#iform > table > tbody > tr > td > table:nth-child(5) > tbody > tr:nth-child(3) > td:nth-child(2) > input[type='text']")
        search.send_keys("%s"%x)
        browser.find_element_by_css_selector("#iform > table > tbody > tr > td > table:nth-child(5) > tbody > tr:nth-child(9) > td:nth-child(2) > input[type='submit']:nth-child(1)").click()
        for a in browser.find_elements_by_css_selector("#iform > table > tbody > tr > td > table:nth-child(6) > tbody > tr > td:nth-child(1) > a"):
            info = []
...
s = requests.session()
#browser.get("https://license.ohio.gov/Lookup/")
for line in codecs.open("OH_eng_Links.csv","r","utf-8"):
    try:
        quotes = "%s" %line
        link = quotes.replace("\"","")
