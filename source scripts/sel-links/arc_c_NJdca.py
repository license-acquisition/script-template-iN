...
page = 1
count = 1
f=codecs.open('arc_c_NJdca_%s%s%s_000.csv' %(str(year), str(month).zfill(2), str(day).zfill(2)), 'w', 'utf-8')

#Define and write headers to CSV.
...
for x in link_list:
        ...
        try:
            ...
            # data = (name + ',', city + ',', state + ',', license_type + ',', license_no + ',', status + ',', cause + ',', issue + ',', exp + '\n')
            if count % 4 == 0:
                print ('good one!')
            else:
                if count % 3 == 0:
