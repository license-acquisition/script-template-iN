...
f = codecs.open('gen.res.a_c_CO.FCcfc_%s_000.csv' %(time.strftime('%Y%m%d')), 'w', 'utf-8')
...
def log(status):
    ...
    feed = 'gen.res.a_c_CO.FCcfc_%s_000.txt' %(time.strftime('%Y%m%d'))  # date in output
...
open('authority.pdf', 'wb').write(requests.get('http://citydocs.fcgov.com/?cmd=convert&vid=2&docid=2438982&dt=MONTHLY+REPORT').content)
  ...
  ########entity_name
      if 1 <= len(nline) <= 4: 
        ...
        if digits:
          ...
          temp_name = ""
        elif "Gen Contractor Roster" in temp_name:
          temp_name =""
      ...
      if len(nline) == 6: 
        ...
        info.append("General Contractor")
        info.append("1")
        info.append("1")
        print '|'.join(info) + '\n'
        f.write('|'.join(info) + '\n')
