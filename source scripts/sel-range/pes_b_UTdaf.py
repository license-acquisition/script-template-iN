...
f=codecs.open("pes_b_UTdaf_%s_000.txt" %(time.strftime('%Y%m%d')),"w", 'utf-8')
f.write('|'.join(["Name","License Type","License Number","Customer Number","Mailing Address", "Location Address", "Application Date", "Renewal Printed", "First Certification", "Renewal Date", "License Expires", "When Re-Certified", "License Printed", "Certification Expires", "Late Fee Printed", "Uniform Registry Number", "Applicator Name", "Applicator License", "Expired"]) + '\n')
browser = webdriver.Chrome()
browser.get('http://ag.utah.gov/licenses-registrations.html')
browser.find_element_by_xpath('//*[@id="main"]/article/ul[2]/li[1]/a').click() # open instance of sql access
browser.find_element_by_xpath('/html/body/form/table/tbody/tr[54]/td[1]/input').click()

for i in range(1,2000):
        try:
                browser.get('http://webapp.ag.utah.gov/LicenseLookup/pageRedirection.jsp?lic_Number=%s&lic_Type=4000&runPage=licDetails.jsp&LicenseType=4000&sortBy=tCustomerAddress.LastName' %i)
